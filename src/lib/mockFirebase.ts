...
class MockDb {
  ...
  private getData(collectionName: string): any[] {
    try {
      ...
      console.warn(`Resetting invalid local data for ${collectionName}`, error);
      try { localStorage.removeItem(this.getStorageKey(collectionName)); } catch {}
      return [];
    }
  }
  ...
  async collection(name: string) { return name; }
  async doc(collectionName: string, id: string) { return { collectionName, id }; }
  ...
  async setDoc(docRef: any, data: any, options?: { merge?: boolean }) {
    ...
    if (index !== -1 && options?.merge) newData = { ...collectionData[index], ...data, id: docRef.id };
    if (index !== -1) collectionData[index] = newData;
    else collectionData.push(newData);
  ...
  async updateDoc(docRef: any, data: any) { return this.setDoc(docRef, data, { merge: true }); }

  async deleteDoc(docRef: any) {
    const collectionData = this.getData(docRef.collectionName);
    this.setData(docRef.collectionName, collectionData.filter(i => i.id !== docRef.id));
  }
        ...
        try {
          ...
          if (query.filters) {
            query.filters.forEach((f: any) => {
              if (f.op === '==') filteredData = filteredData.filter(i => i[f.field] === f.value);
            });
          }
          ...
          if (query.limitCount) filteredData = filteredData.slice(0, query.limitCount);
          callback({
          ...
          console.error('Mock snapshot error:', error);
          errorCallback?.(error);
        }
      };
  ...
  query(collectionName: string, ...constraints: any[]) {
    ...
    constraints.forEach(c => {
      ...
      if (c.type === 'orderBy') { q.orderByField = c.field; q.orderDirection = c.direction; }
...
class MockAuth {
  ...
  constructor() {
    try {
      ...
      console.warn('Invalid mock auth data; resetting guest session.', error);
    }
    if (!this.user) {
      this.user = { uid: 'guest_user', email: 'guest@example.com', displayName: 'Guest User' };
      try { localStorage.setItem('mock_auth_user', JSON.stringify(this.user)); } catch {}
    }
  }
  ...
  async signInWithPassword(role: string, pass: string) {
    if (role === 'admin' && pass === 'Admin@MA2024') {
      this.user = { uid: 'admin_user', email: 'aaliofficialy@gmail.com', displayName: 'Aali (Admin)' };
      try { localStorage.setItem('mock_auth_user', JSON.stringify(this.user)); } catch {}
  ...
  async signInWithPopup() { return { success: false, error: 'Password authentication required for Admin Portal' }; }
